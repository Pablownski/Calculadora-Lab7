...
const isError = (value: string) => value === ERROR_TEXT

export const Display = ({ value }: DisplayProps) => {
  const error = isError(value)
  return (
    <div
      className="w-full px-4 py-3 flex items-end justify-end overflow-hidden"
      ...
      <AnimatePresence mode="wait">
        <motion.span
          ...
          className={[
            ...
            value.length > 7 ? 'text-2xl' : 'text-4xl',
            error ? 'text-red-400 drop-shadow-[0_0_12px_rgba(248,113,113,0.8)]' : 'text-white'
          ].join(' ')}
        >
          {value}
        </motion.span>
